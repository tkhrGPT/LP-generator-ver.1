...
## SEC-TR-06: 比較表セクション（trust）

```
pattern_id        : SEC-TR-06
parent_template   : trust
section_role      : 自社サービスと「一般的な選択肢」の差を視覚的に対比し、「なぜこれを選ぶべきか」を論理的に示す
suitable_personas : skeptic, result-seeker
key_emotion       : 「確かにここが違う」という納得
tone              : clean, bold, reassuring
avoid_when        : 他社商品名を直接比較する場合（景表法リスク）。premium × quality-first（比較すること自体が世界観と合わない）
```

### 構成要素

```
セクション見出し : 「何と何を比べているか」が一目でわかるタイトル
比較軸          : 3〜6項目（価格・内容・サポート・継続性など）
比較相手        : 「一般的なサービス」「従来の方法」「独学」など抽象化した対比（実在の競合名は使わない）
視覚表現        : ○/×、または色分けによる明快な差異表示
CTA            : 比較表の直後に配置（「違いがわかった」直後の行動意欲をキャッチ）
```
...
- ペルソナが `skeptic` で「比較して決めたい」タイプのとき
- `category: course / app` との相性が高い（機能・サポートが比較しやすい）
- SEC-TR-04（特徴セクション）の後に置くと証拠 → 比較 → 納得の流れが生まれる
- 比較軸は「相手が弱い項目」ではなく「自社が強い項目」から選ぶ
...
### SEC-CMN-04: ロードマップ・ステップセクション

```
pattern_id        : SEC-CMN-04
section_role      : 「この商品を使うとどんな順序で変化が起きるか」を段階的に示し、成果へのイメージを具体化する
suitable_personas : result-seeker, skeptic, aspiration-driven
key_emotion       : 「見通しが立った」「自分にもできそう」
tone              : clean, energetic, reassuring
avoid_when        : cosmetics / food のようにステップが説明しにくい商材。story テンプレートの感情的な流れの途中に置くと流れを断ち切る
```

#### 構成要素

```
セクション見出し : ロードマップ全体を示すタイトル
ステップカード   : STEP1〜3（または〜5）の番号付き構成
各ステップ      : ステップ名（短く）+ 説明（1〜2行）+ 期間・目安（あれば）
視覚的な流れ    : 矢印または番号で「順序」が一目でわかるレイアウト
```
...
- `category: course / app` と相性が高い
- ペルソナが `skeptic` のとき: 「具体的な手順がある」ことが信頼になる
- SEC-TR-03（根拠セクション）の後 or SEC-CMN-02（フルCTA）の前に置くと流れが良い
- ステップ数は3〜5が最適（2つでは薄く、6つ以上は読まれにくい）

---

### SEC-CMN-05: 対象者明確化セクション

```
pattern_id        : SEC-CMN-05
section_role      : 「この商品が向いている人・向いていない人」を明示することで、読者の自己同一化を促し、信頼感と購買確度を同時に高める
suitable_personas : skeptic, result-seeker, aspiration-driven
key_emotion       : 「これは自分のことだ」「このブランドは正直だ」
tone              : clean, bold, reassuring
avoid_when        : 幅広い層に訴求する必要がある場合（絞り込みすぎると機会損失）。food / cosmetics の汎用商材（除外設定がなじみにくい）
```

#### 構成要素

```
セクション見出し : 「こんな方に」または「こんな方には向いていません」
向いている人リスト: 3〜5項目。ターゲットペルソナの具体的な状況・悩みで描写する
向いていない人リスト: 2〜3項目。正直に書くことで信頼感が増す（誇大でなく事実を書く）
つなぎコピー    : 「当てはまる方には、ぜひ続きを読んでください」など
```
...
- HOOK-08（対象者絞り込み型）と組み合わせると最大効果
- `category: course` での高単価サービスでは必須に近い
- ペルソナが `skeptic` のとき: 「選別している = 質が高い」という印象を作る
- 向いていない人リストは自社の弱点ではなく「そもそもターゲットではない人」を書く
