...
```
pattern_id        : CTA-U03
pattern_name      : 希少性・アクセス限定型
summary           : 価格ではなく「このコンテンツ・機会そのものへのアクセスが限られる」という希少性で行動を促す。アーカイブなし・次回未定・人数上限などが根拠
suitable_categories: course
suitable_personas : result-seeker, skeptic, aspiration-driven
suitable_price_band: low, mid, high（無料でも機能する）
suitable_lp_goals : signup, trial
suitable_templates: trust, story
tone              : clean, bold, reassuring
avoid_when        : 希少性が事実でない場合（使用禁止）。cosmetics / food / app（アクセス制限が機能しにくいカテゴリ）
```

### ボタン文言パターン

```
[メインCTA]  参加する / 申し込む（今回限り）
[限定性テキスト]  アーカイブ・録画配信は行いません
              今後この内容が公開されることはありません
              次回開催は未定です
```
...
- 価格割引でなく「情報・体験へのアクセス」が商品価値の中心のとき
- `lp_goal: signup` かつ無料〜低価格オファーのとき（希少性が申込みの最後の一押し）
- ペルソナが `skeptic` のとき: 売りつけていない感が強く逆に信頼される
- CTA-U01/U02（価格限定型）と使い分ける:「価格が上がる」→ U01/U02、「機会がなくなる」→ U03

---

## CTA-F01: 無料体験・ファーストステップ型

```
pattern_id        : CTA-F01
pattern_name      : 無料体験・ファーストステップ型
summary           : 「まず無料で体験する」という低ハードルの入口を提示し、高単価サービスへの心理的な壁を下げる
suitable_categories: course, app
suitable_personas : skeptic, pain-avoider, lifestyle-fit
suitable_price_band: high, premium（本命サービスが高い場合）
suitable_lp_goals : trial, inquiry
suitable_templates: trust, story
tone              : soft, warm, reassuring
avoid_when        : 無料体験の準備がない場合。cosmetics / food（無料体験が構造的に難しい）
```

### ボタン文言パターン

```
[メインCTA]  まず無料で試す / 初回無料で始める / 無料相談を予約する
[サブテキスト]  はじめての方は、まず○○から
```
...
- 本命サービスが高単価（月額○万円・年額○万円）のとき、無料体験を入口にする
- `lp_goal: inquiry` のとき第一候補（相談→成約の流れ）
- ペルソナが `skeptic` のとき: 「まず試せる」という安心感が決め手になる
- 「初回無料」を使う場合は、その後の案内フロー（セールス・クロージング）とセットで設計する
...
| 用途 | パターン |
|---|---|
| 無料体験・初回無料のある高単価サービス | CTA-F01 |
| アーカイブなし・次回未定の希少性訴求 | CTA-U03 |
