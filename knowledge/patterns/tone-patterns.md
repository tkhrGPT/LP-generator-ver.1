...
## TONE-08: conversational（会話体・正直さ型）

```
pattern_id        : TONE-08
pattern_name      : conversational
summary           : 話しかけるような一人称、失敗談・本音の開示、自己ツッコミを使って「この人は信用できる」という感情的な同意を先に作る。テクニックより人柄で買わせるトーン
suitable_categories: course
suitable_personas : skeptic, aspiration-driven, result-seeker
suitable_price_band: low, mid, high（無料〜高額まで機能する）
suitable_lp_goals : purchase, signup, trial
suitable_templates: story, trust
tone              : warm, clean（この二つとセットで使う）
avoid_when        : cosmetics / apparel / food（ブランドの世界観を守る必要がある商材）。premium × quality-first（洗練さが崩れる）
```

### 文体指針

```
- 「自分の言葉」で書く。敬語と話し言葉が混在してもいい
- 失敗・迷い・本音をためらいなく書く（ただし関係ある失敗だけ）
- 読み手が「こんなこと考えてたの自分だけじゃなかった」と思える場所を必ず作る
- 自己ツッコミ・ユーモアを1〜2箇所入れると距離が縮まる
- 「誰に向けて書いているか」を明示する（「〇〇な人へ」「私が話したいのは〜」）
- 結論を先に言わず、「なぜこれを書くか」から始めると話についてきやすくなる
```

### 例文（抽象パターン — 商材の言葉で書き直すこと）

```
正直に言います。最初は全然違う考えを持っていました。
笑ってしまう話なのですが、それが転換点だったんです。
なんで今さらこれを書くのか、と思うかもしれません。
シンプルに言うと、聞かれ続けて面倒くさくなったからです。
```
...
```
- テキスト量が多くなるため、見出し・マーカー・ボックスで読みやすく分割する
- フォントはゴシック系、行間はやや広め（1.8〜2.0）
- 背景は白またはごくわずかにオフホワイト
- 装飾は最小限。「文章の力」で読ませる設計
- 画像はビジュアルより「顔写真・実際の写真」が効果的
```
