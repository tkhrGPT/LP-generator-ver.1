...
| hero_style | 推奨フック |
|---|---|
| emotional | HOOK-01, HOOK-02, HOOK-07 |
| bold | HOOK-03, HOOK-04, HOOK-05 |
| simple | HOOK-05（短く・静かに）|

---

## HOOK-07: ストーリー・エピソード型

```
pattern_id        : HOOK-07
pattern_name      : ストーリー・エピソード型
summary           : 実際に起きたエピソード（失敗・葛藤・転換点）から語り始め、「この人は信頼できる」という感情的な同意を先に作る
suitable_categories: course, app
suitable_personas : skeptic, aspiration-driven, result-seeker
suitable_price_band: mid, high, premium
suitable_lp_goals : purchase, trial, signup
suitable_templates: story, trust
tone              : warm, clean, energetic
avoid_when        : ファーストビューに数字・ビジュアルで即決を促す場合（スクロールが必要なため）。food / cosmetics の衝動買い商材には流れが遅すぎる
```
...
[意外・共感・笑いを誘うエピソードの描写]
  ↓
[エピソードが示す「よくある失敗パターン」の一般化]
  ↓
[「だから自分は○○することにした」という転換]
```

### 例文（抽象パターン — 商材の言葉で書き直すこと）

```
正直に言う。最初は全然うまくいかなかった。
笑ってしまうくらい間抜けな失敗だったけど、それが転換点だった。
「こんなことで？」と思うかもしれないが、それが本質だった。
```

### 選定ロジック

- `hero_style: emotional` + `structure_template: story` のとき
- ペルソナが `skeptic` で「言葉より人を信用する」タイプのとき特に効果的
- 語り手の権威を「実績」でなく「正直さ」で確立したいとき
- ユーモアや失敗談を含む場合、堅苦しくない商材（課題解決系コース・アプリ）で機能する
- 長文LPやセールスレター形式との相性が高い
...
```
pattern_id        : HOOK-08
pattern_name      : 対象者絞り込み型
summary           : 「これは○○な人のためだけ」と明示することで、ターゲットの自己同一化を促し、オファーへの希少性と適切感を同時に作る
suitable_categories: course, app, supplement
suitable_personas : result-seeker, skeptic, aspiration-driven
suitable_price_band: mid, high, premium
suitable_lp_goals : purchase, signup, inquiry
suitable_templates: trust, story
tone              : clean, bold, reassuring
avoid_when        : 幅広い層に売りたい場合（絞りすぎるとリーチが落ちる）。food / cosmetics の汎用商材
```

### 構造パターン

```
[この商品・サービスが「向いている人」の具体的な描写]
  ↓
[「向いていない人」の明示（逆説的に価値を高める）]
  ↓
[「あなたはどちらですか？」という問いかけ or 自然な誘導]
```

### 例文（抽象パターン — 商材の言葉で書き直すこと）

```
これは、○○を本気で変えたい人のために作りました。
向いていない方には正直にお伝えします。
○○な方には、おそらく合いません。
向いているかどうか、3つの質問で確認できます。
```

### 選定ロジック

- `lp_goal: inquiry` または選別性が高いサービス（コーチング・コース）
- ペルソナが `skeptic` のとき: 「売りつけようとしていない」という信頼を作る
- ペルソナが `result-seeker` のとき: 「自分はまさにこれ」という確信を生む
- SEC-CMN-05（対象者明確化セクション）と組み合わせて使う
